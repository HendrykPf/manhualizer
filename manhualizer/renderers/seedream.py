"""Renderer for the Seedream model via Replicate."""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/08_renderers/seedream.ipynb.

# %% ../../nbs/08_renderers/seedream.ipynb #cell-3
...
# %% auto #0
__all__ = ['SeedreamRenderer']

# %% ../../nbs/08_renderers/seedream.ipynb #cell-4
class SeedreamRenderer(BaseRenderer):
    """Image generation via the Seedream model on Replicate.

    Supports LoRA weights and negative prompts.
    No reference image input — character consistency via text prompts only.

    Requires: REPLICATE_API_TOKEN environment variable.
    Config: renderer.seedream.model_ref (Replicate model string)
    """

    def __init__(self, model_spec: ModelSpec, config: RendererConfig):
        super().__init__(model_spec, config)
        self._model_cfg = config.seedream
    ...
    def _render_sync(self, panel: Panel, output_dir: Path, output_cfg: OutputConfig) -> RenderResult:
        ...
        if not self._model_cfg.model_ref:
            raise ValueError(
                "renderer.seedream.model_ref is not set. "
